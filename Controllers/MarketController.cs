...
using SepoliaNftMarket.Models.EndpointsCursors;
using SepoliaNftMarket.Providers.Alchemy;
using SepoliaNftMarket.Services;
using SepoliaNftMarket.Services.Token;
using SepoliaNftMarket.Services.UpdateNftMetadata;
...
public class MarketController(
    ...
    ApiDbContext db,
    IAlchemyProvider alchemyProvider) : ControllerBase
{
    //[Authorize]
    [HttpGet("user-tokens")]
        ...
        [FromQuery] string orderBy = "desc")
    {
        ...
        if (!string.IsNullOrEmpty(nextCursor))
        {
            ...
            {
                dir = data.OrderBy;
                userTokens = await alchemyProvider.GetUserTokensAsync(address, dir == OrderDirection.Desc);
        ...
        [FromQuery] string direction = "all")
    {
        ...
        {
            var fromMeta = await alchemyProvider
                .GetNftsMetadataAsync(trade.From.NftContracts.ToList(),
